...
export function createGate(config: GateConfig): GateClass {
  const { allowedHosts, fetcher = (request: Request) => fetch(request) } =
    config
  ...
  const Gate = class extends WorkerEntrypoint {
    override async fetch(request: Request): Promise<Response> {
      if (!checkHost(request.url, allowedHosts)) {
        ...
        return new Response(
          `Forbidden: requests to ${hostname} are not allowed`,
          { status: 403 },
        )
      }
      ...
      if (!isSecureScheme(request.url)) {
        ...
        return new Response(
          `Forbidden: refusing to attach credentials over ${protocol}`,
          {
            status: 403,
          },
        )
