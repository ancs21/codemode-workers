...
import { registerCodemodeTools, type RegisteredTool, type ToolRegistrar } from '../src/tools'
...
function captureRegistrar() {
	...
	const server: ToolRegistrar = {
		registerTool(name, config, cb) {
			tools.set(name, { config: config as RegisteredTool, cb: cb as ToolCallback })
		}
	}
	return { server, tools }
}
...
describe('registerCodemodeTools', () => {
	it('registers search and execute with descriptions and schemas', () => {
		...
		expect(tools.get('search')?.config.description).toContain('Fake API spec for tests')
		expect(tools.get('execute')?.config.description).toContain('api.request(')
	})

	it('search runs code against the baked catalog', async () => {
		const tools = setup()
		const out = await tools.get('search')!.cb({ code: 'async () => Object.keys(spec.paths)' })
	...
	it('formats isolate errors as tool errors', async () => {
		const tools = setup()
		const out = await tools.get('execute')!.cb({ code: 'async () => { throw new Error("nope") }' })
		expect(out.isError).toBe(true)
	...
	it('truncates oversized results to the configured cap', async () => {
		...
		const out = await tools.get('search')!.cb({ code: 'async () => "y".repeat(5000)' })
