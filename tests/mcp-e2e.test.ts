...
describe('MCP over Streamable HTTP', () => {
	it('tools/list returns search and execute', async () => {
		...
		expect(r.result?.tools?.map((t) => t.name).sort()).toEqual(['execute', 'search'])
	})
	...
	it('tools/call search runs code over the baked catalog', async () => {
		const r = await rpc({
			...
			id: 2,
			method: 'tools/call',
			params: { name: 'search', arguments: { code: 'async () => Object.keys(spec.paths)' } }
		})
	...
	it('tools/call execute reaches the API through the gate with injected creds', async () => {
		const r = await rpc({
			...
			params: {
				name: 'execute',
				arguments: { code: 'async () => api.request({ method: "GET", path: "/v1/me" })' }
			}
		})
